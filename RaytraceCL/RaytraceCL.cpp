...
void saveImage() {
	...
	//FILE* f = fopen("opencl_raytracer.png", "w");
	//bitmap_image image(200, 200);
	//unsigned char rgb[image_width * image_height * 3];
	//unsigned char *p = rgb;
	//for (int i = 0; i < image_width * image_height; i++)
	//{
	//	*p++ = (unsigned char)toInt(cpu_output[i].s[0]);
	//	*p++ = (unsigned char)toInt(cpu_output[i].s[1]);
	//	*p++ = (unsigned char)toInt(cpu_output[i].s[2]);
	//}

	//svpng(f, image_width, image_height, rgb, 0);
	//fclose(f);
	...
	for (int y = 0; y < image_height; y++)
	{
		...
		{
			image.set_pixel(x, y, (unsigned char)toInt(cpu_output[i].s[0]), (unsigned char)toInt(cpu_output[i].s[1]), (unsigned char)toInt(cpu_output[i].s[2]));
			i++;
		}
	}
...
void main() {
	...
	// save image
	saveImage();
	cout << "Saved image to 'image.bmp" << endl;
